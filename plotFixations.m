...
% function requires a mat files that has variables specified
    ...
    for i = 1:length(xdatlist); %for each of the 92 types
        ...
        if(unique(fixtab(fixidxs,1))~=unique(fixtab_nodrift(fixidxs_nodrift,1)))
            fprintf('drift corrected and uncorrected are on different trials! how??!\n');
        end
        ...
        calcRoiFixIdx = find(cell2mat(data.fix(:,1))==ilabtrial);
        ...
        scatter(fixtab(fixidxs,2),fixtab(fixidxs,3),fixtab(fixidxs,7),'fill','r')
        ...
        % mark dot and lines
        legend({'fix_{corrected}','data_{actual}','fix_{actual}'})
